...
_QUOTAS = "SELECT minutes, messages, agents, concurrent_calls FROM quotas WHERE org = $1"

# Replaced whole: a limit the operator left out is NULL, which is no limit.
_SET_QUOTAS = """
INSERT INTO quotas (org, minutes, messages, agents, concurrent_calls, set_at)
    VALUES ($1, $2, $3, $4, $5, now())
    ON CONFLICT (org) DO UPDATE
    SET minutes = excluded.minutes, messages = excluded.messages, agents = excluded.agents,
        concurrent_calls = excluded.concurrent_calls, set_at = now()
"""

...
STATE = "state.changed"
# The code a page showed that the call claimed (api/calls/codes.py), carried so the next socket's
# view knows the caller is also on the site instead of reading "no" until the call ends.

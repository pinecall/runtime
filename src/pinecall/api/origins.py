...
# minted (api/calls/room_token.py): the call's log, its folded state, its recording. Those three
# answer ANY origin, because what opens them is the bearer the page brings — a token for that one
# call — and never a cookie: a page on another site reads nothing it did not bring the token for.
# GET only, no credentials, and the headers a player seeks with and a stream resumes with.
#
# A code's standing (api/calls/codes.py) is the same kind of read: the page that shows four digits
# asks whether a call has claimed them, with the code token its server was handed and nothing else.

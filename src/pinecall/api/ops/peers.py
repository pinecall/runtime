...
def get_sandbox_peer(connection: HTTPConnection, settings: SettingsDep) -> Peer | None:
    ...
    if settings.world != PRODUCTION or not settings.sandbox_url or not settings.sandbox_key:
        return None
    return Peer(get_http(connection), settings.sandbox_url, settings.sandbox_key)
...
def get_production_peer(connection: HTTPConnection, settings: SettingsDep) -> Peer | None:
    ...
    if settings.world != SANDBOX or not settings.identity_url or not settings.peer_key:
        return None
    return Peer(get_http(connection), settings.identity_url, settings.peer_key)


SandboxDep = Annotated["Peer | None", Depends(get_sandbox_peer)]
ProductionDep = Annotated[Peer | None, Depends(get_production_peer)]

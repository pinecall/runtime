...
class Doors:
    """Which corner of a world picks up an agent's ring, and whose phone dials in.

    WHICH agent answers a door is not here and is not this process's to remember: a door is a row
    in `routes` that an operator typed, and it outlives every socket. What is here only ever makes
    sense beside a live socket.

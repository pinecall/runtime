"""An agent's synthetic callers: listed, written and dropped, as the world's and not the code's."""
...
def the_personas(connection: HTTPConnection) -> Personas:
    """Where this org's callers are kept. The one door that asks for them is this file."""
...
async def personas(slug: str, key: EvalsKeyDep, kept: PersonasDep) -> PersonaList:
    """Every caller written for this agent. One list an agent, whichever world asks for it."""
    return PersonaList(personas=[Persona(**one) for one in await kept.of(key.org, slug)])
...
async def put_persona(
    slug: str, name: str, said: PersonaPut, key: EvalsKeyDep, kept: PersonasDep
) -> PersonaList:
...
async def drop_persona(slug: str, name: str, key: EvalsKeyDep, kept: PersonasDep) -> PersonaList:
    """The caller's row gone, and the list after. A name nobody wrote is a 404 that says so."""
    try:
        return PersonaList(
            personas=[Persona(**one) for one in await kept.drop(key.org, slug, name)]
        )
    except NoSuchPersona as nobody:
        raise HTTPException(404, str(nobody)) from nobody

...
from pinecall.api.sso import HandshakesDep, HttpDep, SsoDep
...
# password would open — not disabled, not on its own provider — and the switcher does the rest.
# A row still INVITED is seated: Google verified the address, which is exactly what the link in
# the invitation would have proved, and it is what an org's own provider does (api/login_sso.py).
# Every invited row of theirs is seated, not only the oldest: they proved the address once.

...
| `PINECALL_SIGNUP_KEY` | the `Bearer` key the sign-up doors take: only the page that runs a bot shield in front of them (pinecall.io, with Pineward) holds it, and behind it the client is the `X-Pinecall-Client` it sends (a header of our own: Caddy rewrites `X-Forwarded-For`). Unset, the doors take anybody. A secret: on a box, `make secret NAME=PINECALL_SIGNUP_KEY`, read by the gateway as a credential |

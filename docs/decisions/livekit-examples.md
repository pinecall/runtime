...
| `turn_handling.interruption` `resume_false_interruption` + `false_interruption_timeout` | `voice_agents/basic_agent.py:91-92` | **ours-too** | tk-c020cd. Resume is already livekit's default (`voice/turn.py:195`); only the number is a decision, and the example's `1.0` is the one we took over the library's `2.0`. |
...
| `tts_text_transforms` | `voice_agents/basic_agent.py:100-104`, `homepage/agent.py:82` | **ours-too** | tk-c020cd. The two filters are already the default (`agent_session.py:351`); they are written because the parameter *replaces* the list, so a tenant's `replace({…})` alone would take the asterisks out loud with it. |
| `stt_context_options.keyterms` | `voice_agents/basic_agent.py:107` | **ours-too** | tk-c020cd, as the tenant's declared `hears` mapped to each vendor's door. |
...
| `expressive` | `starter src/agent.py:130`, `homepage/agent.py:81` | **not-yet** | it injects the TTS provider's markup guide into the LLM prompt, which lands in the static prefix the tenant owns (`prompt-regions.md`). It waits for a voice we ship whose markup it can steer. |
...
| `ctx.log_context_fields` | `voice_agents/basic_agent.py:74`, `starter src/agent.py:98`, `telephony/amd.py:38`, `telephony/basic_dtmf_agent.py:130` | **ours-too** | tk-c020cd, in `worker/entry.py`: every livekit log line of the job names its room, which is the call id. |
...
- **Handoffs and tasks over one long prompt** (`:39-41`) — the same argument as
  `prompt-regions.md`, from the other end.
...
## Publishing a caller's own audio into a room (tk-26205e)
...
shape of the whole feature. See [simulate.md](simulate.md).
...
| `lk agent simulate --scenarios scenarios.yaml`, `instructions` / `agent_expectations` / `userdata` | `data_capture_sim/scenarios.yaml` + the simulations doc page | **not-for-us as a runner, ours as a design** | *"Simulations run on LiveKit Cloud using your project's credentials"* — a self-hosted box has no project. The three ideas are taken and named in [simulate.md](simulate.md); the runner is ours. |

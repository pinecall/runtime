...
a number exists once in a world, and a ring reaches the caller's own copy or the agent's line (§3), even through a production number. **And somebody sees all of them**: `GET /v1/agents` answers a key that opens `team` one row per corner, each carrying `holder` (`null` for the org's own), and the `pinecall-corner` header opens any of them.
...
**A call is the agent's, never a socket's.** The calls the socket that left was serving go to the
socket that would take a new call of that agent in that corner — or wait, parked, for the next
process that registers it (a console takes none). The socket that takes one hears `call.attached`
first: how the call started, its last state, the seq it changed hands at — then every `tool.call`
still unanswered, again. It sends its whole prompt and its tools for that call; the log never kept
their text. A deploy is a process leaving and another arriving, and no call is cut:
...
Thirty lines and one dependency, no Pinecall package: [the-smallest-app.md](the-smallest-app.md).
...
`callback.requested` onto the agent's log and answers `204`, and `GET /v1/callbacks[?agent=&after=]` answers `{requests, next}` — every
...
| `PUT/DELETE /v1/line/from` · `GET /v1/line/numbers` · `GET/POST/DELETE /v1/agents/{slug}/line` | **where a RING lands**, in two steps. (`/v1/line/numbers`, `app`: the phones this person said are theirs, the org's production phone numbers and the agent each reaches — what a developer's phone dials, which the numbers door would not tell a sandbox key; read at production's `GET /v1/routes` on `PINECALL_PEER_KEY`, `503` while the sandbox holds none.) First whose phone dialled: a developer says which number they call FROM (`app`, the sandbox, a key naming a person) and every call they make lands in their own corner — no coordination, three of them testing at once. Then, for a number nobody claimed, the agent's **line**: reading it takes `calls`, claiming and releasing take `app`; the first corner to hold an agent takes it and it is handed on when that terminal closes, instead of the newest `pinecall start` silently answering in a colleague's scrollback. Neither is a row — both are only meaningful next to a socket, and `pinecall start` re-says the phone on every connect. Production has one corner and the box holds it — with one exception, the next row. `TheLine` in `rest.json` |
| `GET /v1/agents/{slug}/rings-for?caller=` | **a production ring from a developer's own phone.** Production's worker asks it on every phone call to a production number that no dispatch aimed (`app`; the fleet's key adds `&org=`): is the phone dialling one a developer registered with `PUT /v1/line/from`, and are they holding this agent in the sandbox, in this org? The claims live on the sandbox's gateway, where that developer's CLI knocks, so production answers from its own table and, when it has nothing, asks its sandbox the same door (`PINECALL_SANDBOX_URL`, on `PINECALL_SANDBOX_KEY`, a fleet key the sandbox minted for it — `pinecall-runtime box peer`). `{holder, fleet}` names that developer and the fleet holding their corner (the answering instance's `PINECALL_FLEET`), and the worker hands the room over: a dispatch into the same room to that fleet, the metadata of any dispatch (`org`, `env: sandbox`, `holder`, `agent`, `caller`, `direction`) plus `diverted_from: production`, and its own job ended before anything was opened. `{holder: null, fleet: null}` is production's, and so is a sandbox that does not answer in two seconds or refuses (one WARNING line), and a dispatch the SFU refuses: the call stays where it rang. Every other caller of the real number reaches production |
...
| `GET /v1/evals/runs?agent=&limit=&since=` · `GET /v1/evals/runs/{id}` | what this gateway has run; `since` is a unix second, for a pipe that reads only what is new |

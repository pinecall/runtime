...
allowed what the gateway's policy says. **A person is their email**, one password across every org. Invitations and resets are **mailed** over generic SMTP — the box's `PINECALL_SMTP_URL`, or the org's own account at `GET/PUT/DELETE /v1/org/mail` — and a forgotten password is asked for at `POST /v1/login/reset`: [people.md](people.md).

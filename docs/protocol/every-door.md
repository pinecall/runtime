...
| `POST` | `/v1/numbers/buy` · `?dry_run=true` | buy one number on the box's own Twilio into the org, wired as an import, capped by the `numbers` quota — or the plan alone; *production's*, and on a sandbox `404` naming `PINECALL_ELSEWHERE_URL` |
...
| `POST` | `/v1/signup/verify` | the mailed code back: the org made, allowed what its gateway's policy says, its admin active, their first key and a login code, `201`; 15 minutes, six tries |
| `POST` | `/v1/signup/resend` | a new code for a sign-up still waiting; `202` whoever asks |
| `GET` | `/v1/limits` | what the key's org may use on this instance and has used — `minutes`, `messages`, `llm_tokens`, `concurrent_calls`, `agents`, `seats`, `numbers`, each `{limit, used}`, `null` no limit — with `lends` and `billing_url`; any key of the org |
| `GET` | `/v1/whoami` | the org as an id AND as the `slug` its people type, the key's id, its label, the world this request runs in (`env`), whether it may act in `production`, its `scopes`, whose it is (`subject`, `name`, and `email` — the one name a person carries into every org, what a service serving them across orgs knows them by), whether that person runs the box (`operator`) and whether they are inside an org they are no member of (`visitor_email`) |

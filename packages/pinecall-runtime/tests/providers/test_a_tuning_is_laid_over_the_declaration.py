...
from pinecall.providers.tuned_declaration import NOT_RUN_HERE, apply_tuning, tuned_llm, tuned_voice
...
def test_nothing_set_is_the_declaration_with_every_knob_at_the_runtimes_default() -> None:
    config = apply_tuning(DECLARED, Tuning(), NOTHING)
...
def test_a_model_knob_reads_three_ways() -> None:
    ...
    both = apply_tuning(DECLARED, Tuning(llm="openai/gpt-5"), NOTHING).llm
    vendor = apply_tuning(DECLARED, Tuning(llm="openai"), NOTHING).llm
    model = apply_tuning(DECLARED, Tuning(llm="claude-sonnet-4-5"), NOTHING).llm
...
def test_a_vendor_this_build_has_no_file_for_is_refused() -> None:
    with pytest.raises(DeclarationRefused, match="no llm vendor named"):
        apply_tuning(DECLARED, Tuning(llm="openai-but-misspelt/gpt-5"), NOTHING)


def test_a_voice_name_becomes_the_id_the_vendor_knows_and_tts_moves_the_stage() -> None:
    named = apply_tuning(DECLARED, Tuning(voice="mateo"), NOTHING).voice
    assert named is not None and named.voice_id == VOICES["mateo"].voice_id
    assert named.provider == "elevenlabs"
    moved = apply_tuning(DECLARED, Tuning(tts="cartesia/sonic-3", voice=A_UUID), NOTHING).voice
    ...
    with pytest.raises(DeclarationRefused, match="no voice named 'a-uuid'"):
        apply_tuning(DECLARED, Tuning(tts="cartesia", voice="a-uuid"), NOTHING)
...
def test_a_model_the_vendor_file_does_not_vouch_for_is_refused() -> None:
    ...
    with pytest.raises(
        ...
        apply_tuning(DECLARED, Tuning(tts="cartesia", tts_model="sonic-9"), NOTHING)
    kept = apply_tuning(DECLARED, Tuning(tts="cartesia/sonic-2"), NOTHING).voice
...
def test_an_elevenlabs_model_this_build_does_not_run_is_refused() -> None:
    ...
    with pytest.raises(DeclarationRefused, match=re.escape(refused)):
        apply_tuning(DECLARED, Tuning(tts="elevenlabs", tts_model="eleven_turbo_v2_5"), NOTHING)


def test_the_opening_is_the_worlds_words_or_nobodys() -> None:
    said = Greeting(say="Buenas.", allow_interruptions=True)
    assert apply_tuning(DECLARED, Tuning(greeting=said), NOTHING).greeting == said
    assert apply_tuning(DECLARED, Tuning(), NOTHING).greeting is None
...
def test_the_lexicon_is_the_agents_says_and_hears() -> None:
    ...
    config = apply_tuning(DECLARED, Tuning(), words)
...
def test_a_voice_call_runs_ten_minutes_unless_the_world_says_otherwise() -> None:
    ...
    assert apply_tuning(DECLARED, Tuning(), NOTHING).max_duration_s == 600
    assert apply_tuning(DECLARED, Tuning(max_duration_s=900), NOTHING).max_duration_s == 900
    assert apply_tuning(DECLARED, Tuning(max_duration_s=0), NOTHING).max_duration_s == 0

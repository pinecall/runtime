...
ASKS_NOTHING_OR_ASKS_INSIDE: dict[str, str] = {
    ...
    "POST /v1/signup": "takes no key but the shield's: it asks for the org, and mails a code",
    "POST /v1/signup/verify": "takes no key but the shield's: the mailed code is the credential",
    "POST /v1/signup/resend": "takes no key but the shield's: it answers the same for anybody",

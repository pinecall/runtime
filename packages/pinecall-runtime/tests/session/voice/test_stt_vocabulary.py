...
def test_an_agent_that_declared_nothing_and_holds_nothing_asks_for_nothing() -> None:
    assert stt_vocabulary.words(AgentConfig(slug="clinica-norte")) == []
...
def test_the_declared_words_come_first_and_the_state_adds_the_names_it_is_holding() -> None:
    ...
    assert stt_vocabulary.words(CLARA, state) == [
        "Clínica Norte",
        "doctora Vidal",
        "Ana García",
        "martes",
...
def test_a_phone_number_is_not_a_word_anybody_pronounces() -> None:
    assert stt_vocabulary.words(AgentConfig(slug="c"), {"phone": "600000001"}) == []
...
def test_a_paragraph_of_notes_is_not_a_keyterm() -> None:
    ...
    assert stt_vocabulary.words(AgentConfig(slug="c"), notes) == []


def test_a_name_the_agent_declared_and_the_state_also_holds_is_asked_for_once() -> None:
    heard = stt_vocabulary.words(CLARA, {"doctor": "doctora Vidal"})
    assert heard == ["Clínica Norte", "doctora Vidal"]


def test_the_list_stops_where_the_vendors_do() -> None:
    crowded = {
        f"seat_{number}": f"paciente {number}" for number in range(stt_vocabulary.MOST_TERMS + 20)
    }
    assert len(stt_vocabulary.words(CLARA, crowded)) == stt_vocabulary.MOST_TERMS


def test_only_ears_that_advertise_the_door_are_ever_handed_keyterms() -> None:
    assert stt_vocabulary.takes_keyterms(SilentEars(keyterms=True)) is True
    assert stt_vocabulary.takes_keyterms(SilentEars()) is False
    assert stt_vocabulary.takes_keyterms(None) is False

...
def test_the_ceiling_is_the_agents_own_when_the_orgs_minutes_are_not_limited() -> None:
    assert time_limit.call_ceiling(600, None) == 600


def test_the_orgs_minutes_end_the_call_first_when_fewer_are_left_than_the_agents_limit() -> None:
    assert time_limit.call_ceiling(600, 90) == 90
    assert time_limit.call_ceiling(600, 3_000) == 600
...
def test_an_agent_with_no_limit_still_ends_where_the_orgs_minutes_do() -> None:
    ...
    assert time_limit.call_ceiling(0, 45) == 45
    assert time_limit.call_ceiling(0, None) == 0
...
def test_the_orgs_minutes_ending_the_call_first_are_written_as_credits_exhausted() -> None:
    clock = time_limit.build_clock(600, Ceiling(seconds=90, minutes=30), "clinica")
...
def test_the_agents_own_limit_coming_first_ends_the_call_with_no_refusal() -> None:
    assert (
        time_limit.build_clock(600, Ceiling(seconds=900, minutes=30), "clinica").exhausted is None
    )
    assert (
        time_limit.build_clock(600, Ceiling(seconds=600, minutes=30), "clinica").exhausted is None
    )
    assert time_limit.build_clock(600, None, "clinica") == time_limit.Clock(600)


def test_a_written_visit_is_ended_by_the_minutes_alone_and_says_so() -> None:
    clock = time_limit.build_clock(0, Ceiling(seconds=45, minutes=30), "clinica")

...
from pinecall.session.greeting import greeting_for
...
def test_a_call_a_person_opened_is_greeted_the_way_the_class_declared() -> None:
    assert greeting_for(THE_WORDS, NOBODY_RAN_IT) is THE_WORDS
...
def test_a_call_a_run_opened_is_not_greeted_at_all() -> None:
    ...
    assert greeting_for(THE_WORDS, A_RUN) is None
...
def test_an_improvised_opening_is_dropped_for_a_run_too() -> None:
    ...
    assert greeting_for(Greeting(reply="saluda y preséntate"), A_RUN) is None


def test_a_class_that_declares_no_greeting_is_unchanged_by_any_of_this() -> None:
    assert greeting_for(None, A_RUN) is None
    assert greeting_for(None, NOBODY_RAN_IT) is None

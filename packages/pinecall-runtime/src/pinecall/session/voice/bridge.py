...
class VoiceBridge:
    ...
    def __init__(
        ...
        # and the `tools` block describes one list: session/lookup_tools.py. Built before the
    ...
    def _tell_the_ears(self, state: Mapping[str, Any]) -> None:
        ...
        live = self._live
        if live is None or not stt_vocabulary.takes_keyterms(live.stt):  # pyright: ignore[reportUnknownMemberType, reportUnknownArgumentType]
            return
        live.update_options(keyterms=stt_vocabulary.words(self.config, state))

...
class NoNumbers(PinecallError):
    """The org imported no number, and a call back is shown as one of its own."""


class NoOutboundHost(PinecallError):
    """The org's SIP peer declared no address to send calls AT."""
...
@dataclass
class Provisioned:
    """What provisioning did, one sentence a step, and the trunk and address once it wrote them."""
...
# `dry` plans and writes nothing: the steps say what a real run would do, in the order it would.
async def provision_trunk(
    ...
    *,
    ...
    carrier = await carriers.of(org)
    if carrier is None:
        raise NoCarrier(NO_CARRIER)
    numbers = await own_numbers(routes, org)
    if not numbers:
        raise NoNumbers(NO_NUMBERS)
    if sfu is None:
        raise NoMediaPlane(NO_LIVEKIT)
    done = Provisioned()
    placing = await _reached(carrier, fleet, twilio, trunks, numbers, done.steps, dry)
    on_the_sfu = TRUNK_NAME.format(fleet=fleet, org=carrier.org)
    done.steps.append(
    ...
    done.steps.append(f"kept     {trunk_id} is this org's outbound trunk")
    done.trunk, done.address = trunk_id, placing.address
    return done
...
def _through_the_peer(peer: SipPeer, numbers: tuple[str, ...], steps: list[str]) -> Placing:
    ...
    if host is None:
        raise NoOutboundHost(NO_OUTBOUND_HOST)
...
async def _credentials(
    ...
    if standing is not None and kept is not None and kept.password is not None:
        ...
        raise CredentialsLost(CREDENTIALS_LOST.format(name=name, account=carrier.named))
    else:

...
- **A sign-up proves its email before any org exists.** `POST /v1/signup` now keeps the sign-up in
  ...
  `X-Pinecall-Client` it sends. A fake email used to make an org and a key at once.
...
- **A restart never cuts a call.** A call is its agent's, not the socket's that answered it: when
  ...
  The reaper ends a call in a room once no **agent** is in it — people left in a room (a caller's
  open tab, a supervisor) kept a dead call live for hours — and takes the room down; and it ends a
  written call no process runs once it has been quiet past its door's wait.

...
make instance NAME=production … SANDBOX=https://sandbox.example.com FORCE=1
                   ...
                   their sandbox copy); `pinecall-runtime sandbox seed` once, on the box
```
...
| `sandbox seed [--from-instance production] [--to-instance sandbox]` | once, at the cutover, as root: production's personas, sandbox knowledge and sandbox tuning copied into the sandbox's database, per org, one line per table; never members, keys, calls or memories; run twice copies nothing twice |
...
| `keys issue · list · revoke` | an org's API keys, the operator's way in: printed once, listed by fingerprint, revoked by UPDATE. `issue --scope … --subject … --name …`: minted in the instance's own world (`pc_live_…` at production, `pc_test_…` at a sandbox; `--env` naming the other is refused), what it may do there (left out, every scope but `fleet`), whose it is — with `--subject` a person's (`pc_…`), whose world each request names. A tenant makes its servers' tokens itself, in the console's Tokens screen |

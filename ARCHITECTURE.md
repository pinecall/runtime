...
3. `POST /v1/calls`: the gateway opens the log, binds the call to an app socket and answers `seconds_left`, what the org's minutes leave it; the worker keeps it on the agent's own clock (`closing_time.the_ceiling`).
...
this process's memory), `auth/throttle.py` (so many tries per name per minute at the password door), `orgs/codes.py` (the codes a page shows beside a phone number: kept on the agent's log, held here, closed lazily when they expire),

...
| `GET /v1/insights` · `/v1/org/judging` · `/v1/agents/{slug}/threads` · `/memory` · `/widget` · `POST /v1/evals/judge/{call}` | **the console's doors** ([docs/protocol/console-api.md](docs/protocol/console-api.md)): a day counted, judging on or off (`JudgedWhen`, `evals/score.py`), the inbox by contact with a person's read cursor, an agent's facts across contacts, the widget's settings (`orgs/widgets.py`), a call judged later — all read off the call index (`log/call_facts.py`, `store/index.py`), never a fold of every log. `api/org/insights.py`, `judging.py`, `threads.py`, `agent_memory.py`, `widget.py`, `evals/judge.py` |
...
7. Hang-up: the bridge writes `call.ended`, asks the gateway to remember the call (`POST /v1/calls/{call}/remember`, under `PINECALL_REMEMBER_BUDGET_S`),
   writes `call.summary`, hands its own log to the `Scorer` (`session/score_step.py` → `evals/score.py`), writes `call.score`, and the worker seals the call.

...
| `GET`·`PUT /v1/agents/{slug}/pipeline/hold-audio` · `…/audio` · `…/played` · `GET /v1/agents/{slug}/hold-audio[/audio]` | **the hold melody**: what a caller hears while a tool runs — the runtime's own (`session/a-new-life.ogg`), off, or a file uploaded as the body and converted once to Opus 48 kHz mono (`session/hold_melody.py`, `hold_audio` 0036). Its own doors, never a field of the settings' whole-set PUT. The worker asks the second pair on the fleet's key, keeps a clip by its hash (`worker/hold.py`), and `session/voice/hold_melody.py` plays it on livekit's `BackgroundAudioPlayer` around each tool's round trip, after a grace AND never before the agent has stopped talking — the model emits the line that announces a tool and the tool call in one response, so on a clock alone the melody came up underneath the agent's own voice every time. `api/agents/hold_melody.py` |

...
| `GET /v1/insights` · `/v1/org/judging` · `/v1/agents/{slug}/threads` · `/memory` · `/widget` · `POST /v1/evals/judge/{call}` | **the console's doors** ([docs/protocol/console-api.md](docs/protocol/console-api.md)): a day counted, judging on or off (`JudgedWhen`, `evals/hangup_score.py`), the inbox by contact with a person's read cursor, an agent's facts across contacts, the widget's settings (`orgs/widgets.py`), a call judged later — all read off the call index (`log/call_facts.py`, `store/index.py`), never a fold of every log. `api/org/insights.py`, `judging.py`, `threads.py`, `agent_memory.py`, `widget.py`, `evals/judge.py` |
...
retrying), `room/` (the room's events as facts — the caller's keypad among them, `dtmf.received`, four tones close together asked of the gateway as a page's code by `room/claiming.py`; invite, mute, remove, send; the DataChannel to

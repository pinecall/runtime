...
| **Persona** | `name` (lower-case words joined by hyphens, as `--persona` takes it), `about`, `goal`, `style`, `facts`, `state`, `author`, `set_at` | `agent_personas` (`0042`), primary key (`org`, `agent`, `name`) | one agent's synthetic callers. It **was** a file of the tenant's project (`test/<agent>/personas/*.ts`), which is why only a terminal standing in that directory could list one and the production console showed none: a persona is not code but a goal, a manner and a handful of facts, the same kind of thing as the voice and the lexicon, so it moved here with them (`0038`). One list per agent per **org**, not per world — a caller is a test, not something a customer hears, and `price-shopper` is written once for both worlds. Written whole (nothing is merged); `was` in the body renames. `orgs/personas.py` is the store, `api/agents/personas.py` the three doors |
...
Twenty-nine tables, forty-four migrations (`migrations/00NN_*.sql`, applied in order by `migrate
...
| `GET /v1/agents/{slug}/personas` · `PUT`·`DELETE …/{name}` | **the agent's synthetic callers** (`evals`): one list an agent per org and not per world, each a goal, a manner and the facts it may state. A PUT writes one whole — nothing merged — and `was` in the body renames the caller it names (`404` for a name nobody wrote, `409` for one somebody else holds); a name is lower-case words joined by hyphens or `422`. `api/agents/personas.py`, `orgs/personas.py` |
...
| 2 | the same, spoken: one simulated caller on a real line, an ElevenLabs voice that is not the agent's, in the agent's declared language, waiting for the greeting before its first line and for each answer to land before the next; an interferer and packet loss if asked | the room is held by the gateway, the call by the worker | `evals/calling.py`, `caller.py`, `speech.py`, `line.py`, `api/evals/voice.py`, `listening.py` |

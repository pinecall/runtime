...
| **Carrier** | `org`, `account`: a **TwilioAccount** (`account_sid`, `user`, `secret`) or a **SipPeer** (`username`, `password`, `addresses` CIDR) | `carriers` (`org`, `kind`, `account`, `ciphertext` under the vault key, `set_at`) | whose numbers reach the org's agents. One per org. Importing a number: the carrier's trunk pointed at the box and the number attached (`routes/twilio.py`), the org's LiveKit inbound trunk admitting it (`routes/trunks.py`), the route. `docs/protocol/numbers.md` |
...
`orgs/sso.py` (one OpenID provider per org, its client secret under the same vault key),

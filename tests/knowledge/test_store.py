...
async def test_a_push_counts_its_chunks_and_a_second_push_replaces_the_first(
    ...
    assert await knowledge.put(org, PRODUCTION, THE_BASE, [CLINICA, TARIFAS]) == 4
    [listed] = await knowledge.bases(org, PRODUCTION)
    assert (listed.base, listed.chunks) == (THE_BASE, 4)
    assert await knowledge.put(org, PRODUCTION, THE_BASE, [TARIFAS]) == 2
    [listed] = await knowledge.bases(org, PRODUCTION)
    assert (listed.base, listed.chunks) == (THE_BASE, 2)
    found = await knowledge.search(org, PRODUCTION, THE_BASE, "horarios turnos")
...
async def test_a_search_finds_a_chunk_by_a_heading_word_the_text_search_stems(
    ...
    await knowledge.put(org, PRODUCTION, THE_BASE, [CLINICA, TARIFAS])
    first, *_rest = await knowledge.search(org, PRODUCTION, THE_BASE, "turno")
...
async def test_a_search_finds_a_chunk_by_words_the_text_search_drops_because_the_vector_keeps_them(
    ...
    await knowledge.put(org, PRODUCTION, THE_BASE, [CLINICA, TARIFAS])
    first, *_rest = await knowledge.search(org, PRODUCTION, THE_BASE, "por con el")
    assert first.heading == "Clínica Norte › Turnos"
...
async def test_a_chunk_both_branches_find_scores_one_and_min_score_drops_the_rest(
    ...
    await knowledge.put(org, PRODUCTION, THE_BASE, [CLINICA, TARIFAS])
    found = await knowledge.search(org, PRODUCTION, THE_BASE, "turnos teléfono")
    ...
    assert len(found) == 4
    kept = await knowledge.search(org, PRODUCTION, THE_BASE, "turnos teléfono", min_score=0.6)
...
async def test_k_caps_what_a_turn_is_handed(knowledge: PgKnowledge, org: str) -> None:
    await knowledge.put(org, PRODUCTION, THE_BASE, [CLINICA, TARIFAS])
    assert len(await knowledge.search(org, PRODUCTION, THE_BASE, "revisión", k=1)) == 1
    assert len(await knowledge.search(org, PRODUCTION, THE_BASE, "revisión", k=3)) == 3


async def test_dropping_a_base_never_pushed_answers_false_and_a_pushed_one_true(
    knowledge: PgKnowledge, org: str
) -> None:
    assert await knowledge.drop(org, PRODUCTION, "nunca") is False
    await knowledge.put(org, PRODUCTION, THE_BASE, [CLINICA])
    assert await knowledge.drop(org, PRODUCTION, THE_BASE) is True
    assert await knowledge.bases(org, PRODUCTION) == []
    assert await knowledge.search(org, PRODUCTION, THE_BASE, "horarios") == []
    assert await knowledge.drop(org, PRODUCTION, THE_BASE) is False
...
async def test_an_org_never_sees_another_orgs_base_of_the_same_name(
    ...
    await knowledge.put(org, PRODUCTION, THE_BASE, [CLINICA])
    await knowledge.put(other, PRODUCTION, THE_BASE, [TARIFAS])
    assert {
        chunk.path for chunk in await knowledge.search(org, PRODUCTION, THE_BASE, "revisión")
    } == {"clinica.md"}
...
async def test_a_push_of_nothing_is_a_base_with_no_chunks(knowledge: PgKnowledge, org: str) -> None:
    assert await knowledge.put(org, PRODUCTION, THE_BASE, []) == 0
...
async def test_a_base_pushed_with_another_model_is_refused_naming_both_and_the_way_out(
    ...
    try:
        ...
        with pytest.raises(WrongModel) as refused:
            await other.search(org, PRODUCTION, THE_BASE, "horarios")
    finally:
        await pool.close()
...
async def test_a_base_nobody_pushed_is_not_a_model_mismatch_but_an_empty_answer(
    ...
    try:
        assert (
            await PgKnowledge(pool, OtherModel()).search(org, PRODUCTION, "nunca", "horarios") == []
        )
    finally:
        await pool.close()
...
async def test_the_chunks_an_org_keeps_are_summed_over_its_bases_in_both_worlds(
    ...
    await knowledge.drop(org, DEVELOPMENT, THE_BASE)
...
async def test_a_base_is_one_worlds_and_a_laptops_push_never_touches_the_boxs(
    ...
    written = {chunk.path for chunk in await knowledge.search(org, DEVELOPMENT, THE_BASE, "turnos")}
    ...
    assert await knowledge.drop(org, DEVELOPMENT, THE_BASE) is True
    assert await knowledge.search(org, DEVELOPMENT, THE_BASE, "turnos") == []
    assert len(await knowledge.search(org, PRODUCTION, THE_BASE, "turnos")) == 2

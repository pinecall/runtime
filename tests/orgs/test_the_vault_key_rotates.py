...
from pinecall.orgs.vault import NoVaultKey, build_cipher
...
def test_a_secret_sealed_under_the_old_key_opens_once_the_new_one_is_in_front() -> None:
    sealed_before = build_cipher(OLD).encrypt(b"sk-tenant").decode()
    ...
    assert build_cipher(NEW).decrypt(sealed_now) == b"sk-tenant"

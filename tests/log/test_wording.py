...
def test_the_text_the_model_reads_is_the_error_then_the_summary_then_the_output() -> None:
    assert wording.tool_result_text(ToolResult(call_id="x", name="t", error="no")) == "no"
    summarised = ToolResult(call_id="x", name="t", summary="three slots", output=[1, 2, 3])
    assert wording.tool_result_text(summarised) == "three slots"
    assert (
        wording.tool_result_text(ToolResult(call_id="x", name="t", output={"a": 1})) == "{'a': 1}"
    )
    assert wording.tool_result_text(ToolResult(call_id="x", name="t")) == ""

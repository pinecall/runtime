"""Two instances, one per world: a production key and a sandbox key of ONE org never cross."""
...
def test_each_instance_holds_the_agent_its_own_key_registered_and_the_register_says_which(
    gateway: TestClient, settings: Settings
) -> None:
    """The same slug, held by the box and by the laptop at once: two agents, one per world."""
    with an_app_on(gateway, A_KEY) as deployed:
        ...
        with an_app_on(gateway, A_DEV_KEY) as written:
...
def test_the_sandbox_key_declaring_the_boxs_number_takes_nothing(
    gateway: TestClient, settings: Settings
) -> None:
    with an_app_on(gateway, A_KEY) as deployed:
...
def test_whoami_says_the_world_of_the_instance_each_key_was_made_for(
    gateway: TestClient, settings: Settings
) -> None:
...
def test_a_token_of_one_world_is_refused_at_the_other_worlds_instance(
    gateway: TestClient, settings: Settings
) -> None:
    """Each instance keeps its own keys; were one ever to verify at the other, it opens nothing."""
    status, said = got(gateway, "/v1/whoami", A_DEV_KEY)
    assert status == 403 and said["detail"].startswith("this token was made for sandbox")
    answering_in(SANDBOX, settings)
    status, said = got(gateway, "/v1/whoami", A_KEY)
    assert status == 403 and said["detail"].startswith("this token was made for production")


@pytest.mark.usefixtures("wired")
async def test_the_worker_reads_the_doors_of_its_own_world_and_no_other(
    routes: MemoryRoutes, settings: Settings
) -> None:
...
def test_a_chat_on_the_sandbox_key_is_served_by_the_laptop_and_says_so_on_call_started(
    ...
    with an_app_on(gateway, A_KEY) as deployed:
        ...
        with an_app_on(gateway, A_DEV_KEY) as written:
            ...
            assert deployed.receive_json()["type"] == "pong", "the box heard the laptop's call"
...
class TestATeamInOneWorld:
    ...
    def settings(self, settings: Settings) -> Settings:
        """The sandbox's instance, the one world where a person's key holds an agent."""

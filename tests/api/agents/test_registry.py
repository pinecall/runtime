...
async def test_a_register_names_the_door_the_agent_answers() -> None:
    ...
    await registry.register(A_SOCKET, "madrid", "clinica-norte", [a_door("phone", A_NUMBER)])
...
async def test_the_agents_channels_are_read_off_its_doors() -> None:
    ...
    await registry.register(
        A_SOCKET, "madrid", "clinica-norte", [a_door("phone", A_NUMBER), a_door("web")]
    )
    held = registry.of("clinica-norte")
...
async def test_a_disconnect_frees_the_door_for_whoever_asks_next() -> None:
    ...
    await registry.register(ANOTHER_SOCKET, "madrid", "clinica-sur", [a_door("phone", A_NUMBER)])
    answering = registry.at("phone", A_NUMBER)
    assert answering is not None
...
async def test_a_re_register_on_the_same_socket_keeps_what_the_agent_declared() -> None:
    ...
    registry = Registry(Logs(MemoryStore()))
    await registry.register(A_SOCKET, "madrid", "clinica-norte", [a_door("web")])
    await registry.configure(A_SOCKET, "clinica-norte", defs.AgentConfig(greeting="Clínica Norte."))
    await registry.register(A_SOCKET, "madrid", "clinica-norte", [a_door("phone", A_NUMBER)])
    held = registry.of("clinica-norte")
    assert held is not None
    assert held.config.greeting == "Clínica Norte."
...
async def test_a_configure_only_changes_the_fields_the_app_sent() -> None:
    ...
    await registry.configure(
        A_SOCKET, "clinica-norte", defs.AgentConfig(greeting="Buenos días.", language="es-ES")
    )
    entry = await registry.configure(A_SOCKET, "clinica-norte", defs.AgentConfig(language="es-UY"))
    held = registry.of("clinica-norte")
    assert held is not None
    assert (held.config.greeting, held.config.language) == ("Buenos días.", "es-UY")
...
async def test_the_agents_own_log_carries_both_the_register_and_the_configure() -> None:
    ...
    await registry.register(A_SOCKET, "madrid", "clinica-norte", [a_door("web")])
    await registry.configure(A_SOCKET, "clinica-norte", defs.AgentConfig(greeting="Hola."))
...
async def test_two_agents_of_one_fleet_each_hold_a_web_door_at_once() -> None:
    ...
    registry = Registry(Logs(MemoryStore()))
    await registry.register(A_SOCKET, "madrid", "clinica-norte", [a_door("web")])
    await registry.register(ANOTHER_SOCKET, "madrid", "tienda-sur", [a_door("web")])
    assert registry.of("clinica-norte") is not None
    assert registry.of("tienda-sur") is not None
    answered = [(route.agent, route.channel) for route in registry.routes("madrid")]
...
async def test_a_number_is_still_one_agents_while_both_agents_answer_the_web() -> None:
    ...
    registry = Registry(Logs(MemoryStore()))
    await registry.register(
        A_SOCKET, "madrid", "clinica-norte", [a_door("web"), a_door("phone", A_NUMBER)]
    )
    with pytest.raises(DeclarationRefused, match="already answers for agent clinica-norte"):
        await registry.register(
            ANOTHER_SOCKET, "madrid", "tienda-sur", [a_door("web"), a_door("phone", A_NUMBER)]
        )
...
async def test_two_sockets_hold_one_agent_and_a_new_call_takes_the_newest() -> None:
    registry = Registry(Logs(MemoryStore()))
    await registry.register(A_SOCKET, "madrid", "clinica-norte", [a_door("web")])
    await registry.register(ANOTHER_SOCKET, "madrid", "clinica-norte", [a_door("web")])
    held = registry.of("clinica-norte")
    assert held is not None
    assert held.owner == ANOTHER_SOCKET
    assert registry.on("clinica-norte", A_SOCKET) is not None
    assert registry.on("clinica-norte", "app_nobody") is None
...
async def test_the_agent_stands_while_one_of_its_two_sockets_leaves() -> None:
    ...
    assert [route.channel for route in registry.routes("madrid")] == ["web"]
...
async def test_a_slug_another_fleet_is_holding_is_refused() -> None:
    ...
    with pytest.raises(DeclarationRefused, match="one org"):
        await registry.register(ANOTHER_SOCKET, "barcelona", "clinica-norte", [a_door("web")])
...
async def test_the_register_hands_the_socket_its_own_id_back() -> None:
    ...
    registry = Registry(Logs(MemoryStore()))
    entry = await registry.register(A_SOCKET, "madrid", "clinica-norte", [a_door("web")])
...
async def test_a_call_that_named_no_app_skips_the_socket_that_takes_none() -> None:
    ...
    await registry.register(A_SOCKET, "madrid", "clinica-norte", [a_door("web")])
    await registry.register(
        ANOTHER_SOCKET, "madrid", "clinica-norte", [a_door("web")], takes_unclaimed=False
    )
    serving = registry.serving("clinica-norte", None)
...
async def test_the_socket_that_takes_no_unclaimed_call_is_still_a_full_holder() -> None:
    ...
    registry = Registry(Logs(MemoryStore()))
    await registry.register(
        A_SOCKET, "madrid", "clinica-norte", [a_door("web")], takes_unclaimed=False
    )
    held = registry.of("clinica-norte")
    assert held is not None
    assert held.owner == A_SOCKET
    ...
    named = registry.serving("clinica-norte", A_SOCKET)
...
async def test_nobody_serves_a_call_of_an_agent_only_such_sockets_hold() -> None:
    ...
    assert registry.serving("clinica-norte", None) is None

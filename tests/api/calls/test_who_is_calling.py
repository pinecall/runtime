...
from pinecall.api.calls.chat import build_call_from_socket
...
def test_a_chat_that_names_nobody_is_a_call_memory_files_under_nothing() -> None:
    said = build_call_from_socket(_asked({"agent": AGENT}), "clinica", PRODUCTION, AGENT, "UTC")
...
def test_a_played_caller_carries_its_own_rule_and_a_person_carries_none() -> None:
    ...
    nobody = build_call_from_socket(_asked({"agent": AGENT}), "clinica", PRODUCTION, AGENT, "UTC")

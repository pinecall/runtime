...
from pinecall.whatsapp.webhook_signature import signed
...
def test_the_body_meta_signed_is_taken() -> None:
    ...
    assert signed(AN_APP_SECRET, A_BODY, header) is True


def test_no_header_at_all_is_refused() -> None:
    assert signed(AN_APP_SECRET, A_BODY, None) is False
...
def test_a_header_without_the_prefix_is_refused() -> None:
    ...
    assert signed(AN_APP_SECRET, A_BODY, bare) is False
...
def test_a_body_signed_with_another_secret_is_refused() -> None:
    ...
    assert signed(AN_APP_SECRET, A_BODY, header) is False
...
def test_a_body_changed_after_it_was_signed_is_refused() -> None:
    ...
    header = a_signature(A_BODY)[SIGNATURE_HEADER]
    assert signed(AN_APP_SECRET, A_BODY + b" ", header) is False

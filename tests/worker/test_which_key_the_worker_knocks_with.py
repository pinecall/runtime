...
def _settings(
    url: str = LOCAL, worker_key: str | None = None, dev_key: str | None = None
) -> Settings:
    """A settings object carrying only what this rule reads."""
    return Settings.model_construct(gateway_url=url, worker_key=worker_key, dev_key=dev_key)
...
def test_a_box_knocks_with_the_key_that_was_issued_to_it() -> None:
    ...
    assert the_key_for(_settings(A_BOX, worker_key=AN_ORG_KEY), NO_DOOR) == AN_ORG_KEY
...
# The bug this test exists for: the worker key exported in the shell, a gateway running on a dev
...
def test_that_door_takes_the_dev_key_even_when_an_org_key_is_exported() -> None:
    ...
    assert the_key_for(_settings(worker_key=AN_ORG_KEY), THE_LOCAL_DOOR) == A_DEV_KEY
...
def test_a_door_left_by_another_gateway_changes_nothing_for_a_box() -> None:
    ...
    assert the_key_for(_settings(A_BOX, worker_key=AN_ORG_KEY), THE_LOCAL_DOOR) == AN_ORG_KEY
...
def test_a_local_gateway_with_a_database_and_no_door_takes_the_org_key() -> None:
    ...
    assert the_key_for(_settings(worker_key=AN_ORG_KEY), NO_DOOR) == AN_ORG_KEY

...
from pinecall.session.first_entries import arrival_entry, started_entry
...
def test_call_started_says_which_world_the_call_ran_in() -> None:
    ...
    written = started_entry(a_context(SANDBOX), A_NUMBER, 1.5)
    ...
    assert started_entry(a_context(PRODUCTION), A_NUMBER, 1.5).env == PRODUCTION


def test_the_arrival_is_ringing_for_an_inbound_call_and_dialing_for_an_outbound_one() -> None:
    ringing, offered = arrival_entry(a_context(PRODUCTION), A_NUMBER)
    dialing, placed = arrival_entry(a_context(PRODUCTION, "outbound"), A_NUMBER, asked_by="m_ana")
...
def test_the_two_ends_swap_with_the_direction() -> None:
    _, offered = arrival_entry(a_context(PRODUCTION), A_NUMBER)
    _, placed = arrival_entry(a_context(PRODUCTION, "outbound"), A_NUMBER, asked_by="m_ana")
    ...
    said = started_entry(a_context(PRODUCTION, "outbound"), A_NUMBER, 1.5).model_dump(by_alias=True)
...
def test_call_started_carries_the_callers_own_rule_and_a_persons_call_carries_none() -> None:
    ...
    written = started_entry(ruled, A_NUMBER, 1.5)
    assert (written.accepts_when, written.declines_when) == ("a price", "later")
    nobody = started_entry(a_context(PRODUCTION), A_NUMBER, 1.5)

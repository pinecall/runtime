...
def test_the_orgs_minutes_ending_the_call_first_are_written_as_credits_exhausted() -> None:
    clock = closing_time.the_clock(600, Ceiling(seconds=90, minutes=30), "clinica")
...
def test_the_agents_own_limit_coming_first_ends_the_call_with_no_refusal() -> None:
    assert (
        closing_time.the_clock(600, Ceiling(seconds=900, minutes=30), "clinica").exhausted is None
    )
    assert (
        closing_time.the_clock(600, Ceiling(seconds=600, minutes=30), "clinica").exhausted is None
    )
    assert closing_time.the_clock(600, None, "clinica") == closing_time.Clock(600)


def test_a_written_visit_is_ended_by_the_minutes_alone_and_says_so() -> None:
    clock = closing_time.the_clock(0, Ceiling(seconds=45, minutes=30), "clinica")
...
async def test_the_refusal_is_written_before_the_call_ends_and_not_after() -> None:
    ...
    await closing_time.keep(90, Live(), ended, lambda: False, Clock().sleep, before_the_end=written)

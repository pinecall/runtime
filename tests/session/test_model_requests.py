...
from pinecall.session.model_requests import NotAsking, WhatWasAsked
...
# Every tenant tool is a raw-schema tool: the app runs the body, so there is no Python function to
# introspect (session/tool_declaration.py). A reader that only knew livekit's decorated kind wrote
